...
/// Tests that detailed token usage (including cached and reasoning tokens) is
/// properly parsed from the Chat Completions SSE stream.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn chat_mode_detailed_token_usage() {
    if std::env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
    ...
    let sse = concat!(
        ...
        "data: {\"choices\":[{\"delta\":{}}]}\n\n",
        "data: {\"usage\":{\"prompt_tokens\":125,\"completion_tokens\":75,\"total_tokens\":200,",
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("test"), "Response should contain 'test'");
